...
struct ScopedEventMute
{
	ScopedEventMute(Unigine::EventBase &in_event)
		: event(in_event), prev_enabled(in_event.isEnabled())
	{
		if (prev_enabled)
	...
	ScopedEventMute(const ScopedEventMute &) = delete;
	ScopedEventMute &operator=(const ScopedEventMute &) = delete;

	Unigine::EventBase &event;
	bool prev_enabled;
};

#define MUTE_EVENT(event) Unigine::ScopedEventMute UNIGINE_CONCATENATE(tmp, __LINE__){event};
}
