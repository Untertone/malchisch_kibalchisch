...
public:
	...
	bool load(const Ptr<Stream> &stream, size_t read_size, bool binary = false, bool skip_errors = false);
	bool save(const Ptr<Stream> &stream, bool binary = false, const char *indent = "\t") const;
