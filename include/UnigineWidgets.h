...
typedef const WidgetExternBase ConstWidgetExternBase;
	...
	virtual void setGui(const Ptr<Gui> &new_gui);
	virtual bool getIntersection(int local_pos_x, int local_pos_y) const;
	virtual void addChild(const Ptr<Widget> &widget, int flags = -1);
	virtual void removeChild(const Ptr<Widget> &widget);
	virtual void replaceChild(const Ptr<Widget> &widget, const Ptr<Widget> &old_widget, int flags = -1);
	virtual bool isChild(const Ptr<Widget> &widget) const;
	virtual int getNumChildren() const;
	virtual Ptr<Widget> getChild(int num) const;
	...
	virtual void raise(const Ptr<Widget> &widget);
	virtual void show(const Ptr<Widget> &widget, int x, int y, int show_x = 1, int show_y = 1);
...
protected:
	...
	void invoke_set_time(const WidgetPtr &engine_widget, float time);
	void invoke_update(const WidgetPtr &engine_widget, float ifps);
	void invoke_check_callbacks(const WidgetPtr &engine_widget, int x, int y);
	void invoke_key_press(const WidgetPtr &engine_widget, unsigned int key);
	void invoke_text_press(const WidgetPtr &engine_widget, unsigned int unicode);
	void invoke_show(const WidgetPtr &engine_widget, const WidgetPtr &widget, int x, int y, int show_x = 1, int show_y = 1);
	void invoke_expand(const WidgetPtr &engine_widget, int width, int height);
	void invoke_update_positions(const WidgetPtr &engine_widget);
	void invoke_destroy(const WidgetPtr &engine_widget);
	void invoke_pre_render(const WidgetPtr &engine_widget);
	void invoke_render(const WidgetPtr &engine_widget, int x0, int y0, int x1, int y1);
	void invoke_render(const WidgetPtr &engine_widget);
	void invoke_overlap(const WidgetPtr &engine_widget);
private:
