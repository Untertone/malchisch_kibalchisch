...
class UNIGINE_EDITOR_API PluginManager final : public QObject
{
	Q_OBJECT
public:
	enum PLUGIN_LOAD_FLAGS
	{
		PLUGIN_LOAD_FLAGS_NONE = 0,
		PLUGIN_LOAD_FLAGS_REMEMBER_STATE = 1 << 0,
	};
	...
	/// (PluginInfo states LOADED and RUNNING).</summary>
	static bool loadPlugin(PluginInfo *plugin_info, PLUGIN_LOAD_FLAGS flags = PLUGIN_LOAD_FLAGS_NONE);
	...
	static bool unloadPlugin(PluginInfo *plugin_info, PLUGIN_LOAD_FLAGS flags = PLUGIN_LOAD_FLAGS_NONE);

	/// <summary> Calls `unloadPlugin` and removes plugin_info from the list of plugins.</summary>
	static bool removePlugin(PluginInfo *plugin_info, PLUGIN_LOAD_FLAGS flags = {});
...
private:
	...
	friend class ::UnigineEditor::PluginManagerPrivate; // initialization.
};
