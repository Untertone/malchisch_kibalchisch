...
struct alignas(16) BoundSphere
{
	...
	UNIGINE_INLINE void set(const vec3 &center_, float radius_) { data.set(center_, radius_); }
	...
	UNIGINE_INLINE void setTransform(const mat4 &transform)
	{
		float r = data.w;
			...
			__m128 res_0 = _mm_mul_ps(col_0, _MM_SWIZZLE(data.vec, X, X, X, X));
			__m128 res_1 = _mm_mul_ps(col_1, _MM_SWIZZLE(data.vec, Y, Y, Y, Y));
			__m128 res_2 = _mm_mul_ps(col_2, _MM_SWIZZLE(data.vec, Z, Z, Z, Z));
			data = _mm_add_ps(_mm_add_ps(res_0, res_1), _mm_add_ps(res_2, transform.sse.v3));

			__m128 l2_0 = _mm_length2_ps3(col_0);
			__m128 l2_1 = _mm_length2_ps3(col_1);
			__m128 l2_2 = _mm_length2_ps3(col_2);
			__m128 max_l2 = _mm_max_ss(_mm_max_ss(l2_0, l2_1), l2_2);

			data.w = r * _mm_cvtss_f32(_mm_sqrt_ss(max_l2));
		#else
			...
			float scale = sqrt(max(max(x, y), z));
			radius = r * scale;
		#endif
	}
	/// <summary>Sets the given transformation matrix to bounding sphere.</summary>
	...
	UNIGINE_INLINE void expandRadius(const vec3 *points, int num_points)
	{
		if (isValid())
		{
			for (int i = 0; i < num_points; i++)
			{
				float r2 = length2(center - points[i]);
				if (radius * radius < r2)
					radius = sqrt(r2);
			}
		} else
...
struct alignas(16) BoundBox
{
	...
	UNIGINE_INLINE bool insideValid(const vec3 &point, float radius) const
	{
			...
			return _mm_comile_ss(_mm_dot3(delta, delta), _mm_set_ss(radius * radius)) ? true : false;
		#else
			vec3 direction;
...
struct alignas(16) BoundFrustum
{
	...
	UNIGINE_INLINE void set(const mat4 &modelview_projection)
	{
		...
		// camera
		// imodelview_projection * vec4(0, 0, 1, 1)
		camera.x = imodelview_projection.m02 + imodelview_projection.m03;
		camera.y = imodelview_projection.m12 + imodelview_projection.m13;
		camera.z = imodelview_projection.m22 + imodelview_projection.m23;
		camera /= imodelview_projection.m32 + imodelview_projection.m33;
	...
	UNIGINE_INLINE bool insideShadowValid(const BoundSphere &object, const BoundSphere &light, const vec3 &offset) const
	{
		...
		float distance2 = length2(direction);
		if (distance2 < pow2(object.radius + Consts::EPS))
			return true;
...
struct alignas(16) WorldBoundSphere
{
	...
	UNIGINE_INLINE void expand(const dvec3 &point)
	{
		if (isValid())
		{
			...
			if (length2 > radius * radius)
			{
				...
				double delta = (length - radius) * 0.5;
	...
	UNIGINE_INLINE void expandRadius(const dvec3 &point)
	{
		if (isValid())
		{
			double r2 = length2(center - point);
			if (radius * radius < r2)
				radius = sqrt(r2);
		} else
		{
	...
	UNIGINE_INLINE void expandRadius(const dvec3 *points, int num_points)
	{
		if (isValid())
		{
			for (int i = 0; i < num_points; i++)
			{
				double r2 = length2(center - points[i]);
				if (radius * radius < r2)
					radius = sqrt(r2);
			}
		} else
