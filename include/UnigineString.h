...
public:
	...
	static constexpr int STACK_CAPACITY = 8;

	enum MATCH_FLAGS
	{
		MATCH_FLAGS_NOESCAPE = 1 << 0,    // Disable backslash escaping.
		MATCH_FLAGS_PATHNAME = 1 << 1,    // Slash must be matched by slash.
		MATCH_FLAGS_PERIOD = 1 << 2,      // Period must be matched by period.
		MATCH_FLAGS_LEADING_DIR = 1 << 3, // Ignore /<tail> after Imatch.
		MATCH_FLAGS_CASEFOLD = 1 << 4,    // Case insensitive search.
		MATCH_FLAGS_PREFIX_DIRS = 1 << 5, // Unused.
		MATCH_FLAGS_WILDSTAR = 1 << 6,    // Double-asterisks ** matches slash too. It implies 'MATCH_FLAGS_PATHNAME'.

		MATCH_FLAGS_IGNORECASE = MATCH_FLAGS_CASEFOLD,
		MATCH_FLAGS_FILE_NAME = MATCH_FLAGS_PATHNAME,
	};
	...
	UNIGINE_INLINE void setMemory(char *data_, int length_)
	{
		destroy();
...
public:
	...
	UNIGINE_INLINE static bool startsWith(const char *data, const char *str, bool case_sensitive = true, int data_size = -1, int str_size = -1)
	{
		...
		if (str_size > data_size)
			return false;
		return get_ncompare_func(case_sensitive)(data, str, str_size) == 0;
	}
	UNIGINE_INLINE static bool endsWith(const char *data, const char *str, bool case_sensitive = true, int data_size = -1, int str_size = -1)
	{
		...
		if (str_size > data_size)
			return false;
		return get_ncompare_func(case_sensitive)(data + data_size - str_size, str, str_size) == 0;
	}
	...
	UNIGINE_INLINE static uint64_t hash64(const char *str, int size = -1, uint64_t seed = 0x9e3779b97f4a7c15ull)
	{
		if (str == nullptr)
			return 0;
		if (size == -1)
		...
		uint64_t h = seed ^ (0x9e3779b185ebca87ull * size);
		...
		while (size >= 8)
		{
			uint64_t v;
			memcpy(&v, p, 8);
			v ^= 0x9ddfea08eb382d69ull;
			v *= 0xc2b2ae3d27d4eb4full;
			v = (v << 31) | (v >> 33);
			h ^= v;
			h = ((v << 27) | (v >> 37)) * 0x165667b19e3779f9ull + 0x9e3779b97f4a7c15ull;
			p += 8;
			size -= 8;
		}
		...
		h = ((h << 23) | (h >> 41)) * 0x9ddfea08eb382d69ull + 0xc4ceb9fe1a85ec53ull;
	...
	UNIGINE_INLINE static uint64_t hash64(const String &str) { return hash64(str.get(), str.size()); }
	...
	static bool match(const char *pattern, const char *str, int flags = MATCH_FLAGS_WILDSTAR);
