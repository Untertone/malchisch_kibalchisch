...
struct alignas(16) ivec4
{
	...
	/// <summary>Constructor. Initializes the vector using a given two-component ivec2 source vector: x=v.x, y=v.y, z=0, w=1.</summary>
	UNIGINE_INLINE explicit ivec4(const ivec2 &v) : x(v.x), y(v.y), z(0), w(1)
	{
		UNIGINE_ASSERT_ALIGNED16(this);
	}
	...
	UNIGINE_INLINE void set(const ivec2 &val, int z_, int w_)
	{
		x = val.x;
		y = val.y;
		z = z_;
	...
	/// <summary>Sets the vector equal to the specified source vector.</summary>
	UNIGINE_INLINE void set(const ivec3 &val, int w_)
	{
		x = val.x;
		y = val.y;
		z = val.z;
