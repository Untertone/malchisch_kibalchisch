...
// declare enum and friendly string for PROP_PARAM(Switch, name, ENUMNAME_STR);
#define DECLARE_ENUM_PROP_SWITCH(ENUMNAME, ...) \
	enum ENUMNAME { __VA_ARGS__ }; \
	static constexpr const char* ENUMNAME##_STR = #__VA_ARGS__;

// SOFT ASSERT
// Prints warning with method and node name/id.
// Does NOT disable component.
//
// softassert#1: COMPONENT_SOFT_ASSERT(condition, action);
// softassert#2: COMPONENT_SOFT_ASSERT(condition, "message", action);
//
// If message is not specified, default message is: "expected: <condition>"

// REQUIRE
// Prints error, disables component, then executes action.
//
// require#1: COMPONENT_REQUIRE(condition, action);
// require#2: COMPONENT_REQUIRE(condition, "message", action);
//
// If message is not specified, default message is: "required: <condition>"

// Action can be a block, for example:
// COMPONENT_REQUIRE(camera, "Camera not found", {
//     init_failed = true;
//     return;
// });
#define __COMPONENT_EVAL(X) X

#define __COMPONENT_SOFT_ASSERT_3(expr, error_message, action)		\
	if (!(expr))													\
	{																\
		Unigine::Log::warning("%s: \"%s\" (node id:%d name:\"%s\")",\
				__UNIGINE_FUNCTION__,								\
				(error_message),									\
				node->getID(),										\
				node->getName());									\
		action;														\
...
#define __COMPONENT_SOFT_ASSERT_2(expr, action) \
	__COMPONENT_SOFT_ASSERT_3(expr, "expected: " #expr, action)

#define __GET_MACRO_3_ARGUMENTS(_1, _2, _3, NAME, ...) NAME

#define COMPONENT_SOFT_ASSERT(...)							\
	__COMPONENT_EVAL(__GET_MACRO_3_ARGUMENTS(__VA_ARGS__,	\
		__COMPONENT_SOFT_ASSERT_3,							\
		__COMPONENT_SOFT_ASSERT_2, 0)						\
	(__VA_ARGS__))

#define __COMPONENT_REQUIRE_3(expr, error_message, action)			\
	if (!(expr))													\
	{																\
		Unigine::Log::error("%s: \"%s\" (node id:%d name:\"%s\")",	\
			__UNIGINE_FUNCTION__,									\
			(error_message),										\
			node->getID(),											\
			node->getName());										\
...
#define __COMPONENT_REQUIRE_2(expr, action) \
	__COMPONENT_REQUIRE_3(expr, "required: " #expr, action)

#define COMPONENT_REQUIRE(...)																				\
	__COMPONENT_EVAL(__GET_MACRO_3_ARGUMENTS(__VA_ARGS__, __COMPONENT_REQUIRE_3, __COMPONENT_REQUIRE_2, 0)	\
	(__VA_ARGS__))
...
public:
	...
	void unregisterComponent()
	{
		...
		for (auto it = components.begin(); it != components.end(); ++it)
		{
			...
			for (int i = 0; i < count; i++)
			{
				if (C *c = dynamic_cast<C *>(it->data[i]))
				{
					if (!c->shutdown_called && c->initialized && c->all_init_called)
						c->call_shutdown();
	...
	UNIGINE_API void initializeComponents(const NodePtr &node);
...
public:
	...
	UNIGINE_INLINE void setHidden(bool value) { hidden = value; }
...
public:
	...
	UNIGINE_API const Property *operator->() const;
...
public:
	...
	UNIGINE_API const Material *operator->() const;
	UNIGINE_INLINE int isEmpty() const { return get().get() == nullptr; }
	virtual int nullCheck() const override { return isEmpty(); }

protected:
	virtual String get_value_as_string() const override { return value_guid.makeString(); }
	mutable MaterialPtr value;
	UGUID value_guid;
};
...
public:
	...
	UNIGINE_API const Curve2d *operator->() const;
	UNIGINE_API void save(const XmlPtr &parameter) const override;
	UNIGINE_INLINE int isEmpty() const { return get().get() == nullptr; }
	virtual int nullCheck() const override { return isEmpty(); }
...
public:
	...
	UNIGINE_INLINE C const *operator->() const { return &value; }
...
protected:
	...
	C *getComponentInWorld(bool enabled_only = false) const
	{
		return ComponentSystem::get()->getComponentInWorld<C>(enabled_only);
	}
	...
	void getComponentsInWorld(Unigine::Vector<C *> &out_components, bool clear_vector = false) const
	{
		ComponentSystem::get()->getComponentsInWorld<C>(out_components, clear_vector);
	}
