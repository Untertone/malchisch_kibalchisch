...
struct Hasher<Type *>
{
	...
	UNIGINE_INLINE static HashType create(const Type *t)
	{
		return murmur3Mixer(reinterpret_cast<HashType>(t));
	}
};

template<typename Ret, typename ... Args>
struct Hasher<Ret (*)(Args...)>
{
	using HashType = unsigned long long;
	using FunctionType = Ret (*)(Args...);
	UNIGINE_INLINE static HashType create(FunctionType t)
	{
		return murmur3Mixer(reinterpret_cast<HashType>(t));
	}
};
...
public:
	...
	template<typename K>
	UNIGINE_INLINE Element *findFastByHash(HashType hash, const K &key) const
	{
		Counter pos;
		return data && do_find(hash, key, pos) ? &Policy::element(get_slot_data()[pos]) : nullptr;
	}
