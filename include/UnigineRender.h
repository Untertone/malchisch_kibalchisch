...
public:
	...
	enum STREAMING_IGPU_VRAM_MODE
	{
		STREAMING_IGPU_VRAM_MODE_SYSTEM,
		STREAMING_IGPU_VRAM_MODE_SIZE,
		STREAMING_IGPU_VRAM_MODE_BALANCE,
	};
	...
	enum SHOW_VERTEX_DENSITY_MODE
	{
		SHOW_VERTEX_DENSITY_MODE_DISCRETE_THRESHOLD = 0,
		SHOW_VERTEX_DENSITY_MODE_GRADIENT,
	};

	enum SHOW_QUAD_OVERDRAW_DISPLAY_MODE
	{
		SHOW_QUAD_OVERDRAW_DISPLAY_MODE_GRADIENT = 0,
		SHOW_QUAD_OVERDRAW_DISPLAY_MODE_DISCRETE_THRESHOLD,
	};

	enum SHOW_QUAD_OVERDRAW_PASSES
	{
		SHOW_QUAD_OVERDRAW_PASSES_ALL = 0,
		SHOW_QUAD_OVERDRAW_PASSES_GEOMETRY,
		SHOW_QUAD_OVERDRAW_PASSES_GEOMETRY_WITHOUT_DEPTH_PRE_PASS,
	};
	...
	enum PASS
	{
		...
		PASS_VISUALIZER_QUAD_OVERDRAW,
		PASS_VISUALIZER_QUAD_OVERDRAW_NO_DEPTH_PREPASS,
		PASS_DEFERRED,
		PASS_AUXILIARY,
		PASS_EMISSION,
		PASS_REFRACTION,
	...
	{
		SHADING_QUALITY_LOW = 0,
		SHADING_QUALITY_MEDIUM,
	...
	enum RENDER_UPSCALE_ORDER
	{
		RENDER_UPSCALE_ORDER_BEFORE_POST_EFFECTS = 0,
		RENDER_UPSCALE_ORDER_AFTER_ADAPTATION_COLOR,
		RENDER_UPSCALE_ORDER_BEFORE_TAA,
		RENDER_UPSCALE_ORDER_AFTER_POST_EFFECTS,
		RENDER_UPSCALE_NUM_ORDERS,
	};
	...
	enum RENDER_FSR_MODE
	{
		...
		RENDER_FSR_MODE_NATIVE_AA,
		RENDER_FSR_MODE_CUSTOM,
	...
	enum RENDER_PARAMETER
	{
		RENDER_PARAMETER_FLOAT,
		RENDER_PARAMETER_FLOAT2,
		RENDER_PARAMETER_FLOAT3,
		RENDER_PARAMETER_FLOAT4,
		RENDER_PARAMETER_INT,
		RENDER_PARAMETER_INT2,
		RENDER_PARAMETER_INT3,
		RENDER_PARAMETER_INT4,
		RENDER_PARAMETER_BOOL,
	};
	...
	{
		SystemInfo::MemoryStatistics memory;
	...
	static void memoryClear(bool soft = false);
	static int getAPI();
	...
	static Event<> &getEventBeginVisualizerQuadOverdraw();
	static Event<> &getEventEndVisualizerQuadOverdraw();
	...
	static Event<> &getEventChangedParameters();
	static Event<> &getEventEndFrameExecuteCommandLists();
	static Event<const char *, const char *> &getEventGPUCrashDump();
	...
	static int findParameter(const char *name);
	static int addParameter(const char *name, Render::RENDER_PARAMETER type = Render::RENDER_PARAMETER_FLOAT, bool dynamic = true);
	static void removeParameter(int num);
	static void swapParameters(int num0, int num1);
	static void moveParameter(int from, int to);
	static int cloneParameter(int num);
	static UGUID getParameterGUID(int num);
	static const char *getParameterShaderName(int num);
	static const char *getParameterDefineName(int num);
	static const char *getParameterName(int num);
	static void setParameterName(int num, const char *name);
	static Render::RENDER_PARAMETER getParameterType(int num);
	static void setParameterType(int num, Render::RENDER_PARAMETER type);
	static bool isParameterFloat(int num);
	static bool isParameterDynamic(int num);
	static void setParameterDynamic(int num, bool dynamic);
	static void setParameterFloat(int num, float value);
	static void setParameterFloat2(int num, const Math::vec2 &value);
	static void setParameterFloat3(int num, const Math::vec3 &value);
	static void setParameterFloat4(int num, const Math::vec4 &value);
	static void setParameterInt(int num, int value);
	static void setParameterInt2(int num, const Math::ivec2 &value);
	static void setParameterInt3(int num, const Math::ivec3 &value);
	static void setParameterInt4(int num, const Math::ivec4 &value);
	static void setParameterBool(int num, bool value);
	static void setParameterFloat(const char *name, float value);
	static void setParameterFloat2(const char *name, const Math::vec2 &value);
	static void setParameterFloat3(const char *name, const Math::vec3 &value);
	static void setParameterFloat4(const char *name, const Math::vec4 &value);
	static void setParameterBool(const char *name, bool value);
	static void setParameterInt(const char *name, int value);
	static void setParameterInt2(const char *name, const Math::ivec2 &value);
	static void setParameterInt3(const char *name, const Math::ivec3 &value);
	static void setParameterInt4(const char *name, const Math::ivec4 &value);
	static float getParameterFloat(int num);
	static Math::vec2 getParameterFloat2(int num);
	static Math::vec3 getParameterFloat3(int num);
	static Math::vec4 getParameterFloat4(int num);
	static int getParameterInt(int num);
	static Math::ivec2 getParameterInt2(int num);
	static Math::ivec3 getParameterInt3(int num);
	static Math::ivec4 getParameterInt4(int num);
	static bool getParameterBool(int num);
	static float getParameterFloat(const char *name);
	static Math::vec2 getParameterFloat2(const char *name);
	static Math::vec3 getParameterFloat3(const char *name);
	static Math::vec4 getParameterFloat4(const char *name);
	static int getParameterInt(const char *name);
	static Math::ivec2 getParameterInt2(const char *name);
	static Math::ivec3 getParameterInt3(const char *name);
	static Math::ivec4 getParameterInt4(const char *name);
	static bool getParameterBool(const char *name);
	static int getNumCompiledPSO();
	static int getNumCompiledPSOGlobal();
	...
	static void setShadingQuality(Render::SHADING_QUALITY quality);
	static Render::SHADING_QUALITY getShadingQuality();
	...
	static void setStreamingFreeSpaceRAMCommitted(int committed);
	static int getStreamingFreeSpaceRAMCommitted();
	...
	static void setStreamingIGpuVRAMMode(Render::STREAMING_IGPU_VRAM_MODE mode);
	static Render::STREAMING_IGPU_VRAM_MODE getStreamingIGpuVRAMMode();
	static void setStreamingIGpuVRAMSize(int size);
	static int getStreamingIGpuVRAMSize();
	static void setStreamingIGpuVRAMBalance(int balance);
	static int getStreamingIGpuVRAMBalance();
	...
	static void setOccluderDistance(float distance);
	static float getOccluderDistance();
	...
	static void setSharpenResolution(int resolution);
	static int getSharpenResolution();
	...
	static void setUpscaleOrder(Render::RENDER_UPSCALE_ORDER order);
	static Render::RENDER_UPSCALE_ORDER getUpscaleOrder();
	static void setUpscaleFixFlicker(bool flicker);
	static bool isUpscaleFixFlicker();
	...
	static void setDLSSResolutionScaleValue(float value);
	static float getDLSSResolutionScaleValue();
	static void setDLSSResolutionScaleEnabled(bool enabled);
	static bool isDLSSResolutionScaleEnabled();
	...
	static void setFSRCustomResolutionScale(float scale);
	static float getFSRCustomResolutionScale();
	static void setFSRShowDebugView(bool view);
	static bool isFSRShowDebugView();
	...
	static void setEnvironmentDitherScale(float scale);
	static float getEnvironmentDitherScale();
	static void setEnvironmentHazeDitherScale(float scale);
	static float getEnvironmentHazeDitherScale();
	...
	static void setEnvironmentHazeResolution(int resolution);
	static int getEnvironmentHazeResolution();
	...
	static void setLightsDitherScale(float scale);
	static float getLightsDitherScale();
	...
	static void setCloudsEnvironmentTexturePath(const char *path);
	static const char *getCloudsEnvironmentTexturePath();
	static void setCloudsEnvironmentSky(bool sky);
	static bool isCloudsEnvironmentSky();
	static void setCloudsEnvironmentSkyColor(const Math::vec4 &color);
	static Math::vec4 getCloudsEnvironmentSkyColor();
	static void setCloudsEnvironmentColor(const Math::vec4 &color);
	static Math::vec4 getCloudsEnvironmentColor();
	...
	static void setShowProjAndOmniShadowCasters(bool casters);
	static bool isShowProjAndOmniShadowCasters();
	static void setShowAlphaTest(bool test);
	static bool isShowAlphaTest();
	static void setShowDepthPrePass(bool pass);
	static bool isShowDepthPrePass();
	...
	static void setShowTransparentGBuffer(bool gbuffer);
	static bool isShowTransparentGBuffer();
	static void setShowTransparentLightingAmbient(bool ambient);
	static bool isShowTransparentLightingAmbient();
	static void setShowTransparentLightingEnvironmentProbe(bool probe);
	static bool isShowTransparentLightingEnvironmentProbe();
	static void setShowTransparentLightingVoxelProbe(bool probe);
	static bool isShowTransparentLightingVoxelProbe();
	static void setShowTransparentLightingPlanarProbe(bool probe);
	static bool isShowTransparentLightingPlanarProbe();
	static void setShowTransparentLightingLightOmni(bool omni);
	static bool isShowTransparentLightingLightOmni();
	static void setShowTransparentLightingLightProj(bool proj);
	static bool isShowTransparentLightingLightProj();
	static void setShowTransparentLightingLightWorld(bool world);
	static bool isShowTransparentLightingLightWorld();
	...
	static void setShowVertexDensityEnabled(bool enabled);
	static bool isShowVertexDensityEnabled();
	static void setShowVertexDensityMode(Render::SHOW_VERTEX_DENSITY_MODE mode);
	static Render::SHOW_VERTEX_DENSITY_MODE getShowVertexDensityMode();
	static void setShowVertexDensityDepthTest(bool test);
	static bool isShowVertexDensityDepthTest();
	static void setShowVertexDensitySearchArea(int area);
	static int getShowVertexDensitySearchArea();
	static void setShowVertexDensityThreshold(int threshold);
	static int getShowVertexDensityThreshold();
	static void setShowVertexDensityBlend(float blend);
	static float getShowVertexDensityBlend();
	static void setShowQuadOverdrawEnabled(bool enabled);
	static bool isShowQuadOverdrawEnabled();
	static void setShowQuadOverdrawDisplayMode(Render::SHOW_QUAD_OVERDRAW_DISPLAY_MODE mode);
	static Render::SHOW_QUAD_OVERDRAW_DISPLAY_MODE getShowQuadOverdrawDisplayMode();
	static void setShowQuadOverdrawPasses(Render::SHOW_QUAD_OVERDRAW_PASSES passes);
	static Render::SHOW_QUAD_OVERDRAW_PASSES getShowQuadOverdrawPasses();
	static void setShowQuadOverdrawWireframe(bool wireframe);
	static bool isShowQuadOverdrawWireframe();
	static void setShowQuadOverdrawLandscapeTerrain(bool terrain);
	static bool isShowQuadOverdrawLandscapeTerrain();
	static void setShowQuadOverdrawWaterGlobal(bool global);
	static bool isShowQuadOverdrawWaterGlobal();
	static void setShowQuadOverdrawThreshold(int threshold);
	static int getShowQuadOverdrawThreshold();
	static void setShowQuadOverdrawBlend(float blend);
	static float getShowQuadOverdrawBlend();
	...
	static void setMaterialReplaceFunc(Unigine::Render::MaterialReplaceFunc func);
	static Unigine::Render::MaterialReplaceFunc getMaterialReplaceFunc();
};
...
public:
	...
	enum RENDER_MODE
	{
		...
		RENDER_MODE_TRIANGLE_PATCHES,
		RENDER_MODE_QUAD_PATCHES,
		NUM_RENDER_MODES,
	};
	...
	static int render(RenderState::RENDER_MODE mode, int base, int begin, int end, int num);
	static void setVertexBuffer(int num, const Ptr<StructuredBuffer> &buffer, int offset = 0, int num_vertex = -1);
	static void setIndexBuffer(const Ptr<StructuredBuffer> &buffer, int offset = 0, int num_indices = -1);
	...
	static void setConservativeRasterizationEnabled(bool enabled);
	static bool isConservativeRasterizationEnabled();
	...
	static void setTexture(int binding, int slot, const Ptr<Texture> &texture);
	static void setTexture(int binding, int slot, const Ptr<Texture> &texture, int sampler_flags);
	static void setStructuredBuffer(int slot, const Ptr<StructuredBuffer> &buffer);
	static Ptr<Texture> getTexture(int slot);
	static Ptr<StructuredBuffer> getStructuredBuffer(int slot);
...
public:
	...
	static void renderMesh(const Ptr<MeshRender> &mesh, const Ptr<Material> &material, const char *pass_name, const Math::Mat4 &transform, const Ptr<Camera> &camera, Node::TYPE node_type = Node::ANY_TYPE);
};
