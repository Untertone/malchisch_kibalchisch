...
- **Timeline Animation** — a sequencer / timeline system for animating arbitrary object properties (node transforms, material parameters, property values) over time using keyframed curves. Timeline animations are stored in `*.utrack` and `*.uplay` files and played back via [NodeAnimationPlayback](../../../api/library/nodes/class.nodeanimationplayback_cpp.md).
